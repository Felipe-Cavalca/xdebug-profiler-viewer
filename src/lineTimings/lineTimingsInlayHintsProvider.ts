...
export const SHOW_LINE_TIMING_DETAILS_COMMAND =
  "xdebugProfileViewer.showLineTimingDetails";

export class LineTimingsInlayHintsProvider
  implements vscode.InlayHintsProvider, vscode.Disposable
{
  ...
  public provideInlayHints(
    ...
    const latestByLine = this.traceIndex.getLatestLineStatsByDocument(
      document.uri,
    );
...
export async function showLineTimingDetailsCommand(
  ...
  if (!stat) {
    void vscode.window.showInformationMessage(
      "Sem detalhes de tempo para esta linha nos traces indexados.",
    );
    return;
  }
...
function buildLineTimingDetailsHtml(
  stat: TraceLineStats,
  documentUri: vscode.Uri,
  line: number,
): string {
...
function mapStatsToCurrentDocument(
  ...
  for (const stat of stats) {
    const mapped =
      findBestCurrentLineForStat(stat, document, usedLines, lineRemapRadius) ??
      stat.line;
...
function isLineTimingsEnabled(): boolean {
  return vscode.workspace
    ...
    .get<boolean>("lineTimings.enabled", true);
}

function getMinDurationMs(): number {
  return vscode.workspace
    .getConfiguration("xdebugProfileViewer")
    .get<number>("lineTimings.minDurationMs", 0);
}

function getShowLoopsAsAggregate(): boolean {
  return vscode.workspace
    .getConfiguration("xdebugProfileViewer")
    .get<boolean>("lineTimings.showLoopsAsAggregate", true);
}

function getMaxHintsPerFile(): number {
  return vscode.workspace
    .getConfiguration("xdebugProfileViewer")
    .get<number>("lineTimings.maxHintsPerFile", 200);
}

function getLineRemapRadius(): number {
  const raw = vscode.workspace
    .getConfiguration("xdebugProfileViewer")
    .get<number>("lineTimings.lineRemapRadius", 120);
