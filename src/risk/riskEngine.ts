...
export interface BreakageRiskResult {
  status: "known" | "unknown";
  percent?: number;
  details?: string;
  metrics?: BreakageRiskMetrics;
}

interface GraphCache {
  maxInboundCost: number;
  inboundCostById: Map<string, number>;
}

export interface BreakageRiskMetrics {
  fanIn: number;
  fanOut: number;
  inboundCost: number;
  callsObserved: number;
  callsEffective: number;
  cpuAvgPerCall?: number;
  memAvgPerCall?: number;
  timeAvgPerCall?: number;
  cpuEvent?: string;
  memEvent?: string;
  timeEvent?: string;
}
...
export class RiskEngine {
  private readonly graphCacheByProfile = new WeakMap<
    CachegrindProfile,
    GraphCache
  >();

  public computeBreakageRisk(
    profile: CachegrindProfile,
    fn: CachegrindFunction,
  ): BreakageRiskResult {
    ...
    const cpuAvgPerCall =
      callsEffective > 0 && cpuEvent
        ? getEventSelf(fn, cpuEvent) / callsEffective
        : undefined;
    const memAvgPerCall =
      callsEffective > 0 && memEvent
        ? getEventSelf(fn, memEvent) / callsEffective
        : undefined;
    const timeAvgPerCall =
      callsEffective > 0 && timeEvent
  ...
  private getGraphCache(profile: CachegrindProfile): GraphCache {
    const existing = this.graphCacheByProfile.get(profile);
    if (existing) {
      return existing;
    }

    const inboundCostById = new Map<string, number>();
    ...
    const cache: GraphCache = { maxInboundCost, inboundCostById };
    this.graphCacheByProfile.set(profile, cache);
    return cache;
  }
}

function getEdgeInclusive(edge: CachegrindEdge, eventName: string): number {
...
function getEventSelf(fn: CachegrindFunction, eventName: string): number {
...
function getEventInclusive(fn: CachegrindFunction, eventName: string): number {
  const eventCost = fn.eventCosts[eventName];
  if (eventCost && Number.isFinite(Number(eventCost.inclusive))) {
    return Number(eventCost.inclusive);
  }
...
function computeChurnRisk(
  ...
  const inboundNorm =
    maxInboundCost > 0 ? inboundCost / maxInboundCost : degreeNorm;
  const score =
    100 * (0.45 * fanInNorm + 0.25 * degreeNorm + 0.3 * inboundNorm);
...
function pickCpuEvent(events: string[]): string | undefined {
  return events.find(
    (eventName) =>
      /cpu|time|wall|duration|ns|us|ms|sec/i.test(eventName) &&
      !/mem|byte/i.test(eventName),
  );
}

function pickMemoryEvent(events: string[]): string | undefined {
  return events.find((eventName) => /mem|byte/i.test(eventName));
}

function pickTimeEvent(
  events: string[],
  primaryEvent: string,
): string | undefined {
  if (
    /time|ns|us|ms|sec|wall|duration/i.test(primaryEvent) &&
