  ...
  for (const rawLine of content.split(/\r?\n/)) {
    ...
    if (tabular) {
      ...
      events.push({
        ...
        durationUs,
        memoryDeltaBytes: computeMemoryDelta(
          pending.enterMemoryBytes,
          tabular.memoryBytes,
        ),
...
export function validateDevSampleTrace(): void {
  ...
  if (result.events.length < 2) {
    throw new Error(
      `Invalid dev trace sample: expected at least 2 events, got ${result.events.length}`,
    );
  }
  const first = result.events[0];
  if (!first.filePath || !first.line || first.durationUs <= 0) {
    throw new Error(
      "Invalid dev trace sample: first event must have file, line and positive duration.",
    );
  }
}
...
function parseFunctionWithArgs(raw: string): {
  functionName: string;
  argsPreview?: string;
} {
  const text = raw.trim();
...
function sanitizeArgsPreview(raw: string | undefined): string | undefined {
  ...
  const masked = compact.replace(
    SENSITIVE_ARG_RE,
    (_, key: string, sep: string) => `${key}${sep}***`,
  );
