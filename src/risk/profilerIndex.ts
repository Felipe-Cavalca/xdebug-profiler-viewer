...
export class ProfilerIndex implements vscode.Disposable {
  ...
  public async initialize(): Promise<void> {
    ...
    for (const glob of PROFILER_GLOBS) {
      ...
      watcher.onDidCreate(
        (uri) => this.scheduleUpsert(uri, this.getIndexOptions().debounceMs),
        this,
        this.disposables,
      );
      watcher.onDidChange(
  ...
  public async rebuildAll(): Promise<void> {
    ...
    const results = await Promise.all(
      PROFILER_GLOBS.map((glob) =>
        vscode.workspace.findFiles(
          glob,
          "**/node_modules/**",
          FUNCTION_INDEX_SCAN_LIMIT,
        ),
      ),
    );
  ...
  public getLatestProfileForFunction(
    name: string,
    documentUri: vscode.Uri,
    line?: number,
  ): IndexedFunctionMatch | undefined {
    ...
    const candidates = occurrences.filter(
      (entry) => entry.resolvedFilePath === targetPath,
    );
    let best = pickBestOccurrence(candidates, line);
    if (!best) {
      best = pickBestOccurrence(
        occurrences.filter((entry) => !entry.resolvedFilePath),
        line,
      );
    }
    if (!best) {
    ...
    const functionData = profileState.profile.functions.find(
      (fn) => fn.id === best.functionId,
    );
    if (!functionData) {
      return undefined;
    }
  ...
  private async upsertProfile(
    ...
    try {
      ...
      const occurrences = await this.buildOccurrencesForProfile(
        uri,
        fileStat.mtime,
        profile,
      );
  ...
  private async buildOccurrencesForProfile(
    ...
    for (const fn of profile.functions) {
      const normalizedName = normalizeName(
        extractComparableFunctionName(fn.name),
      );
      if (!normalizedName) {
        continue;
      }
      const resolvedFilePath = await this.resolveFunctionFilePath(
        fn.file,
        profileUri,
      );
      occurrences.push({
        ...
        resolvedFilePath,
        rawFilePath: fn.file
          ? normalizeSlashes(fn.file).toLowerCase()
          : undefined,
        line: fn.line,
        functionId: fn.id,
      });
  ...
  private async resolveFunctionFilePath(
