...
export class RiskCodeLensProvider
  implements vscode.CodeLensProvider, vscode.Disposable
{
  ...
  private async getDocumentFunctions(
    document: vscode.TextDocument,
  ...
  private async getFunctionsFromSymbols(
    document: vscode.TextDocument,
  ): Promise<FunctionDescriptor[]> {
...
export function registerRiskCodeLensNoop(
  context: vscode.ExtensionContext,
): void {
  context.subscriptions.push(
    vscode.commands.registerCommand(RISK_NOOP_COMMAND, () => undefined),
  );
  context.subscriptions.push(
    vscode.commands.registerCommand(
      OPEN_PROFILE_COMMAND,
      async (profileUri: vscode.Uri) => {
...
function extractPhpFunctionsByRegex(
  document: vscode.TextDocument,
): FunctionDescriptor[] {
  const out: FunctionDescriptor[] = [];
  const regex =
    /^\s*(?:(?:public|protected|private|static|abstract|final)\s+)*function\s+&?\s*([A-Za-z_][A-Za-z0-9_]*)\s*\(/;
