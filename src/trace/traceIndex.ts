...
interface MutableLineStats {
  ...
  minDurationUs: number;
  maxDurationUs: number;
  functionStatsByName: Map<
    string,
    { totalDurationUs: number; count: number; totalMemoryDeltaBytes?: number }
  >;
  topSlowEvents: TraceLineSlowEvent[];
}
...
export class TraceIndex implements vscode.Disposable {
  ...
  private readonly sourceResolutionCache = new Map<
    string,
    string | undefined
  >();
  ...
  public getLineStatsForDocument(
    documentUri: vscode.Uri,
  ): TraceDocumentStats | undefined {
    ...
    for (const state of this.profileByKey.values()) {
      ...
      if (
        candidate.traceMtime === best.traceMtime &&
  ...
  public getLatestLineStatsByDocument(
    documentUri: vscode.Uri,
  ): Map<number, TraceLineStats> {
    const targetPath = normalizeFsPath(documentUri.fsPath);
    ...
    for (const state of this.profileByKey.values()) {
      ...
      for (const candidate of lines.values()) {
        ...
        if (
          candidate.traceMtime === current.traceMtime &&
  ...
  private rebuildWatchers(): void {
    ...
    for (const glob of this.getTraceGlobs()) {
      ...
      watcher.onDidCreate(
        (uri) => this.scheduleUpsert(uri, options.debounceMs),
        this,
      );
      watcher.onDidChange(
        (uri) => this.scheduleUpsert(uri, options.debounceMs),
  ...
  private async upsertTrace(uri: vscode.Uri, fireEvent = true): Promise<void> {
    ...
    try {
      ...
      const state = await this.buildProfileState(
        uri,
        fileStat.mtime,
        parsed.events,
      );
  ...
  private getIndexOptions(): IndexOptions {
    const cfg = vscode.workspace.getConfiguration("xdebugProfileViewer");
    const debounceMsRaw = cfg.get<number>(
      "codeLens.profilerIndexDebounceMs",
      DEFAULT_INDEX_DEBOUNCE_MS,
    );
    const retryMsRaw = cfg.get<number>(
      "codeLens.profilerIndexRetryMs",
      DEFAULT_INDEX_RETRY_MS,
    );
    const maxRetriesRaw = cfg.get<number>(
      "codeLens.profilerIndexMaxRetries",
      DEFAULT_MAX_INDEX_RETRIES,
    );
    return {
  ...
  private async buildProfileState(
    traceUri: vscode.Uri,
    mtime: number,
    events: TraceCallEvent[],
  ): Promise<TraceProfileState> {
    ...
    for (const event of events) {
      ...
      const resolvedPath = await this.resolveSourcePath(
        event.filePath,
        traceUri,
      );
      if (!resolvedPath) {
      ...
      const currentFunction = bucket.functionStatsByName.get(
        event.functionName,
      ) ?? {
        totalDurationUs: 0,
        count: 0,
    ...
    for (const bucket of mutableByPathLine.values()) {
      const avgDurationUs =
        bucket.count > 0 ? bucket.totalDurationUs / bucket.count : 0;
      ...
      if (existing) {
        ...
        lineStatsBySourcePath.set(
          bucket.normalizedSourcePath,
          new Map([[bucket.line, stat]]),
        );
      }
      hitsBySourcePath.set(
        bucket.normalizedSourcePath,
        (hitsBySourcePath.get(bucket.normalizedSourcePath) ?? 0) + bucket.count,
      );
    }
  ...
  private async resolveSourcePath(
    ...
    const resolved = await this.sourceResolver.resolveSourceUri(
      sourcePath,
      traceUri,
    );
...
function insertTopSlowEvent(
  bucket: TraceLineSlowEvent[],
  event: TraceLineSlowEvent,
): void {
  bucket.push(event);
...
function buildFunctionStats(
  ...
  for (const [functionName, entry] of byName.entries()) {
    const avgDurationUs =
      entry.count > 0 ? entry.totalDurationUs / entry.count : 0;
    const avgMemoryDeltaBytes =
      entry.totalMemoryDeltaBytes !== undefined && entry.count > 0
