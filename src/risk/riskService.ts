...
export class RiskService {
  ...
  public getBreakageRiskDisplay(
    ...
    const match = this.profilerIndex.getLatestProfileForFunction(
      fn.name,
      document.uri,
      fn.line + 1,
    );
    ...
    const result = this.riskEngine.computeBreakageRisk(
      match.profile,
      match.functionData,
    );
...
function formatDisplay(
  ...
  if (metrics?.timeAvgPerCall !== undefined) {
    parts.push(
      `$(clock) ${formatMetric(metrics.timeAvgPerCall, metrics.timeEvent)}/ch`,
    );
  }
  if (
    metrics?.cpuAvgPerCall !== undefined &&
    metrics?.timeAvgPerCall === undefined
  ) {
    parts.push(
      `$(dashboard) ${formatMetric(metrics.cpuAvgPerCall, metrics.cpuEvent)}/ch`,
    );
  }
  if (metrics?.memAvgPerCall !== undefined) {
    parts.push(
      `$(database) ${formatMetric(metrics.memAvgPerCall, metrics.memEvent)}/ch`,
    );
  }
  ...
  if (metrics?.cpuAvgPerCall !== undefined) {
    lines.push(
      `CPU media por chamada: ${formatMetric(metrics.cpuAvgPerCall, metrics.cpuEvent)} (${metrics.cpuEvent})`,
    );
  }
  if (metrics?.memAvgPerCall !== undefined) {
    lines.push(
      `Memoria media por chamada: ${formatMetric(metrics.memAvgPerCall, metrics.memEvent)} (${metrics.memEvent})`,
    );
  }
  if (metrics?.timeAvgPerCall !== undefined) {
    lines.push(
      `Tempo medio por chamada: ${formatMetric(metrics.timeAvgPerCall, metrics.timeEvent)} (${metrics.timeEvent})`,
    );
  }

  lines.push(`Profiler usado: ${path.basename(profileUri.fsPath)}`);
  lines.push(
    `Ultima aparicao da funcao: ${new Date(profileMtime).toLocaleString("pt-BR")}`,
  );
...
function getTimeScaleNs(eventName: string): number | undefined {
  ...
  const inline = text.match(
    /(\d+(?:[.,]\d+)?)\s*(ns|us|ms|s|nsec|usec|msec|sec)\b/i,
  );
  if (inline) {
