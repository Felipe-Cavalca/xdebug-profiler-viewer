...
export function parseCachegrind(content: string): CachegrindProfile {
  ...
  for (const rawLine of content.split(/\r?\n/)) {
    ...
    if (line.startsWith("fn=")) {
      ...
      currentFunction = getOrCreateFunction(
        functions,
        functionName,
        currentFile,
      );
    ...
    if (line.startsWith("cfn=")) {
      ...
      const callee = getOrCreateFunction(
        functions,
        calleeName,
        currentCalleeFile,
      );
    ...
    if (currentEdge) {
      ...
      applyEventCostsToMap(
        currentEdge.eventCosts,
        parsedData.values,
        events,
        false,
      );
  ...
  const functionList: CachegrindFunction[] = Array.from(functions.values()).map(
    (fn) => ({
      id: fn.id,
      name: fn.name,
  ...
  const totalSelf = Number(
    summaryByEvent[primaryEvent] ?? computedSelfTotals[primaryEvent] ?? 0,
  );
  const totalCalls = functionList.reduce(
    (acc, fn) => acc + fn.callsObserved,
    0,
  );
  const maxFanIn = functionList.reduce(
    (acc, fn) => Math.max(acc, fn.callers.length),
    0,
  );
  const maxFanOut = functionList.reduce(
    (acc, fn) => Math.max(acc, fn.callees.length),
    0,
  );
  const maxDegree = functionList.reduce(
    (acc, fn) => Math.max(acc, fn.callers.length + fn.callees.length),
    0,
  );
...
export function validateProfile(
  ...
  if (requireEventSelfMatch) {
    for (const eventName of activeEvents) {
      const expected = profile.functions.reduce(
        (sum, fn) => sum + Number(fn.eventCosts[eventName]?.self ?? 0),
        0,
      );
  ...
  for (const fn of profile.functions) {
    if (fn.self > 0 && fn.callsEffective < 1) {
      errors.push(
        `callsEffective must be >= 1 when self > 0 for function "${fn.name}"`,
      );
    }
    for (const edge of fn.callees) {
      for (const eventName of activeEvents) {
        ...
        if (inclusive < 0) {
          errors.push(
            `edge inclusive must be >= 0 for "${fn.name}" -> "${edge.name}" event "${eventName}"`,
          );
        }
      }
    }
  ...
  if (errors.length > 0) {
    throw new Error(
      `Invalid cachegrind profile invariants:\n- ${errors.join("\n- ")}`,
    );
  }
}
...
function parseDataLine(
  line: string,
): { line?: number; values: number[] } | undefined {
...
function cloneEventCostMap(
  map: Map<string, CachegrindEventCost>,
): Map<string, CachegrindEventCost> {
...
function mapToObject(
  map: Map<string, CachegrindEventCost>,
): Record<string, CachegrindEventCost> {
...
function extractTimeScaleNs(eventName: string): number | undefined {
  ...
  const tuple = text.match(
    /\((\d+(?:[.,]\d+)?)\s*(ns|us|µs|μs|ms|s|nsec|usec|msec|sec)\)/i,
  );
  if (tuple) {
  ...
  const inline = text.match(
    /(\d+(?:[.,]\d+)?)\s*(ns|us|µs|μs|ms|s|nsec|usec|msec|sec)\b/i,
  );
  if (inline) {
